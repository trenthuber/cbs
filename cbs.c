...
int main(int argc, char **argv) {
	...
	while ((current_arg = cbs_shift_args(&argc, &argv))) {
		if (cbs_string_eq(current_arg, "build")) {
			...
			if (cbs_needs_rebuild_file_paths("./build/hello", file_paths))
				cbs_async_run(&procs, "cc", CFLAGS, "-o", "./build/hello", "hello.c");
			cbs_file_paths_clear(&file_paths);
			cbs_async_run(&procs, "mkdir", "-p", "test1");
			cbs_async_run(&procs, "mkdir", "-p", "test2");
			cbs_async_run(&procs, "mkdir", "-p", "test3");
			cbs_async_run(&procs, "mkdir", "-p", "test4");
			cbs_async_wait(&procs);
		} else if(cbs_string_eq(current_arg, "run")) {
			if (!cbs_try_run("./build/hello"))
				cbs_run("./cbs", "build", "run");
		} else if(cbs_string_eq(current_arg, "clean")) {
			cbs_run("rm", "-rf", "build");
			cbs_async_run(&procs, "rmdir", "test1");
			cbs_async_run(&procs, "rmdir", "test2");
			cbs_async_run(&procs, "rmdir", "test3");
			cbs_async_run(&procs, "rmdir", "test4");
			cbs_async_wait(&procs);
		} else {
